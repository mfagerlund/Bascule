...
/// the RL metadata the engine's tensor-only format can't carry. Lives in <c>Bascule.RL</c> and is
...
public static class ModelSerializer
{
    ...
    public static InferencePolicy Load(byte[] data)
    {
        ...
        if (data.Length < 8 || r.ReadInt32() != Magic)
            throw new InvalidOperationException("Not a Bascule.RL model blob (bad magic).");
