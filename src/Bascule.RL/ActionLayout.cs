...
public sealed class ActionLayout
{
    ...
    public ActionLayout(ControlSpec controls)
    {
        ...
        if (n == 0)
            throw new ArgumentException(
                "ControlSpec has no channels — a policy needs at least one control channel.", nameof(controls));
    ...
    /// <summary>Option count of a discrete channel: <c>round(Max − Min) + 1</c> (≥ 2). The span must be a
    /// whole number — a discrete channel maps to integer category indices, so a fractional span (e.g.
    /// 0..1.4) is ambiguous and rejected rather than silently rounded.</summary>
    public static int Categories(ControlChannel ch)
    {
        if (!float.IsFinite(ch.Min) || !float.IsFinite(ch.Max))
            throw new ArgumentException(
                $"Discrete channel '{ch.Name}' has non-finite Min/Max ({ch.Min}..{ch.Max}).");
        float span = ch.Max - ch.Min;
        if (MathF.Abs(span - MathF.Round(span)) > 1e-3f)
            throw new ArgumentException(
                $"Discrete channel '{ch.Name}' span must be a whole number (Min={ch.Min}, Max={ch.Max} → " +
                $"span {span}); a discrete channel maps to integer category indices.");
        int k = (int)MathF.Round(span) + 1;
        if (k < 2)
            throw new ArgumentException(
        ...
        ReadOnlySpan<float> std, ReadOnlySpan<float> logStd, Random rng)
    {
        ...
        for (int k = 0; k < ChannelCount; k++)
        {
            ...
            {
                int c = _contIndex[k];
                float mean = MathF.Tanh(policyOut[p]);   // bound the mean to (-1,1): an unbounded head runs past the
                float act = mean + std[c] * PpoUpdate.NextGaussian(rng);   // rails on railed-optimum tasks -> KL blowup
    ...
    /// <summary>Greedy (deterministic) action for inference: continuous → tanh-bounded mean (-1,1);
    ...
    public void Greedy(ReadOnlySpan<float> policyOut, Span<float> envAction)
    {
        for (int k = 0; k < ChannelCount; k++)
        {
            ...
            if (!_discrete[k])
            {
                envAction[k] = MathF.Tanh(policyOut[p]);   // same bounded mean as Sample (greedy = mean)
            }
            else
            {
                int kK = _polLen[k];
    ...
    public (Tensor logp, Tensor entropy) LogpAndEntropy(Tensor policyOut, float[] actionsFlat, int mb, Tensor logStd)
    {
        ...
        if (ContinuousCount > 0)
        {
            ...
            // Tanh-bounded to (-1,1) — matches Sample/Greedy and stops the mean drifting past the action rails.
            Tensor mean = (AllContinuous ? policyOut : TensorOps.IndexSelect(policyOut, 1, _contPolCols)).Tanh();
